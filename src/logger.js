...
const tagJson = (name, objectOrString) => `[${name}:${(typeof objectOrString === 'object' || Array.isArray(objectOrString)) ? JSON.stringify(objectOrString) : objectOrString}]`;
...
const buildLogEntry = (info) => {
    ...
    try {
        ...
        if (stackContainer.stack) {
            ...
            const firstStackCall = stackLines[stackLines.findIndex(stackCall => stackCall.includes('log.js')) - 1];
            tags.push(tagString('class', capitalize(camelize(basename(firstStackCall).split('.js')[0]))));
        }
    } catch (e) {
        if (info.class) {
    ...
    if (info.tags) {
        tags.push(...Object.keys(info.tags).map(tagName => tagString(tagName, info.tags[tagName])));
    }
...
export default class Log {
    ...
    static request(logLevel, url, queryParams, body = '', headers = {}, data = {}) {
        Log.message(logLevel, 'IncomingRequest', Object.assign(data, {
            request: {
                url, queryParams, body, headers,
            },
        }));
    }

    static response(logLevel, status, body, headers = {}, data = {}) {
        Log.message(logLevel, 'OutgoingResponse', Object.assign(data, { response: { status, body, headers } }));
    }
